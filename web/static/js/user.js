...
;(function($){
    ...
    $.ftxia.user = {
		...
		*/
		...
		{
			//login_bnt
			$('.login_bnt').live('click',function(){
				if(!$.ftxia.dialog.islogin())
				{ 
				return !1; }
				else
				{
		...
		sign: function(){
			$('.sign_btn').live('click',function(){
				...
				$.getJSON(FTXIAER.root + '/?m=sign&a=ajax_sign', function(result){
					...
					if(zt == 1 || zt == '1')
					{
						var dou_calender_tpl=''
											 + '<div class="hide_inner">'
												
												 + '<div class="v_con">'
													  + '<img src="'+result.dialog+'" alt=""><br>'
													 // + '下载app，登录每天<br'>
													  //+ '再获得1次签到机会'
												  + '</div>'
												  + '<div class="v_con2">'
												  + '{HEADER}'
													  //+ '<span class="suss">签到成功，获得<span>2积分</span></span><br>'
													 // + '<span class="suss2">'
														 // + '您已经连续签到两天，再继续签到5天，可额外获得10积分'
													 // + '</span>'
													  + '<a href="/?m=gift&a=index">'
														  + '花积分'
													  + '</a>'
												  + '</div>'
											  + '</div>'
						...
						/*var dou_calender_tpl='<div class="alert_content"><div class="top_tips">'
											...
											+'<tbody>{TABLE}</tbody>'
											+'</table></div>';*/
						var not_sign='<span class="suss">签到成功，获得<span>{TODAY_DOU}积分</span></span><br><span class="suss2">明天再来，就可以领到 {TOMORROW_DOU} 个积分，别忘记哦~</span>';
						var signed='<span class="suss">您今天已经签过到了</span><br><span class="suss2">明天再来，就可以获得{TOMORROW_DOU}积分！</span>';
						...
						$.dialog({id:'sign_success', title:result.msg, content:dou_calender, width:500, padding:'', fixed:true, lock:true});
					}else{
					...
					if(result.status == 1){
						var dou_calender_tpl=''
											 + '<div class="hide_inner">'
												 ...
												 + '<div class="v_con">'
													 // + '<img src="images/v_coupon2.png" alt=""><br>'
													 // + '下载app，登录每天<br'>
													  //+ '再获得1次签到机会'
												  + '</div>'
												  + '<div class="v_con2">'
