...
class huameiwangluocheckitemstool
{
	...
	function fetch($zym_40)
	{
		...
		switch (strtolower($zym_42['scheme'])) {
			...
			case 'https':
				...
				break;
			default:
				$this->error = 'Invalid protocol "' . $zym_42['scheme'] . '"\n';
				return false;
				break;
		}
		return true;
					...
					if ($this->_redirectaddr) {
						if ($this->maxredirs > $this->_redirectdepth) {
							...
							if (preg_match('|^http://' . preg_quote($this->host) . '|i', $this->_redirectaddr) || $this->offsiteok) {
								$this->_redirectdepth++;
								$this->lastredirectaddr = $this->_redirectaddr;
								if (strpos($this->_redirectaddr, '?') > 0) $this->fetch($this->_redirectaddr); else $this->submit($this->_redirectaddr, $zym_32, $zym_34);
							}
						}
					}
					if ($this->_framedepth < $this->maxframes && count($this->_frameurls) > 0) {
	...
	{
		$zym_59 = array('\'<script[^>]*?>.*?</script>\'si', '\'<[\/\!]*?[^<>]*?>\'si', '\'([' . "\xd\xa" . '])[\s]+\'', '\'&(quot|#34|#034|#x22);\'i', '\'&(amp|#38|#038|#x26);\'i', '\'&(lt|#60|#060|#x3c);\'i', '\'&(gt|#62|#062|#x3e);\'i', '\'&(nbsp|#160|#xa0);\'i', '\'&(iexcl|#161);\'i', '\'&(cent|#162);\'i', '\'&(pound|#163);\'i', '\'&(copy|#169);\'i', '\'&(reg|#174);\'i', '\'&(deg|#176);\'i', '\'&(#39|#039|#x27);\'', '\'&(euro|#8364);\'i', '\'&a(uml|UML);\'', '\'&o(uml|UML);\'', '\'&u(uml|UML);\'', '\'&A(uml|UML);\'', '\'&O(uml|UML);\'', '\'&U(uml|UML);\'', '\'ß\'i',);
		...
		while ($zym_14 = fgets($zym_43, $this->_maxlinelen)) {
			...
			if (preg_match('/^(Location:|URI:)/i', $zym_14)) {
				...
				if (!preg_match('|\\:\\/\\/|', $zym_13[2])) {
					...
					if (!preg_match('|^/|', $zym_13[2])) $this->_redirectaddr .= '/' . $zym_13[2]; else $this->_redirectaddr .= $zym_13[2];
	...
	function _httpsrequest($zym_49, $zym_40, $zym_50, $zym_52 = "", $zym_51 = "")
	{
		...
		if (!empty($this->host)) if (!empty($this->port)) $zym_30[] = 'Host: ' . $this->host . ':' . $this->port; else $zym_30[] = 'Host: ' . $this->host;
		...
		if (!empty($zym_52)) {
			if ($zym_52 == 'multipart/form-data') $zym_30[] = "Content-type: $zym_52; boundary=" . $this->_mime_boundary; else $zym_30[] = "Content-type: $zym_52";
		}
	...
	function _connect(&$zym_43)
	{
		...
		if ($zym_43 = fsockopen($zym_27, $zym_28, $zym_29, $zym_24, $this->_fp_timeout)) {
			...
			switch ($zym_29) {
				...
				default:
					$this->error = 'connection failed (' . $zym_29 . ')';
			}
			return false;
		}
	}
