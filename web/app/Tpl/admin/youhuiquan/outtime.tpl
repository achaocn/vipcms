<include file="public:header" />
<!--商品列表-->
<div class="pad_10" >

 <div class="J_tablelist table_list" data-acturi="{:U('youhuiquan/ajax_edit')}">
    <table width="100%" cellspacing="0">
        <thead>
            <tr>
                <th width=25><input type="checkbox" id="checkall_t" class="J_checkall"></th>
                <th><span data-tdtype="order_by" data-field="id">ID</span></th>
                <th width="50" align="center">缩略图</th>
                <th align="left"><span data-tdtype="order_by" data-field="title">商品名称</span></th>
               
                <th width="80"><span data-tdtype="order_by" data-field="price_order">价格(元)</span></th>
				<th width="80"><span data-tdtype="order_by" data-field="price_youhui">券后价(元)</span></th>
                <th width="120"><span data-tdtype="order_by" data-field="end_time">结束时间</span></th>
				<th width="80">{:L('operations_manage')}</th>
            </tr>
        </thead>
    	<tbody>
            <volist name="list" id="val" >
            <tr>
                <td align="center"><input type="checkbox" class="J_checkitem" value="{$val.id}"></td>
                <td align="center">{$val.id}</td>
                <td align="right">
                    <notempty name="val['thumb']"><div class="img_border"><a href="http://item.taobao.com/item.htm?id={$val.num_iid}" target="_blank"><img src="{:changepic($val['thumb'],'item')}" width="32" width="32" class="J_preview" data-bimg="{:changepic($val['thumb'],'item')}"></a></div></notempty>
                </td>
                <td align="left"><span data-tdtype="edit" data-field="title" data-id="{$val.id}" class="tdedit" style="color:{$val.colors};">{$val.buss_name}</span></td>
              
                
                <td align="center" class="red">{$val.price_order}</td>
				<td align="center" class="red">{$val.price_youhui}</td>
                <td align="center">{$val.end_time|frienddate}</td>
				<td align="center"><a href="{:u('youhuiquan/edit', array('id'=>$val['id'], 'menuid'=>$menuid))}">{:L('edit')}</a> | <a href="javascript:void(0);" class="J_confirmurl" data-uri="{:u('youhuiquan/delete', array('id'=>$val['id']))}" data-acttype="ajax" data-msg="{:sprintf(L('confirm_delete_one'),$val['title'])}">{:L('delete')}</a></td>
            </tr>
            </volist>
    	</tbody>
    </table>
    </div>

    <div class="btn_wrap_fixed">
        <label class="select_all mr10"><input type="checkbox" name="checkall" class="J_checkall">{:L('select_all')}/{:L('cancel')}</label>
        <input type="button" class="btn btn_submit" data-tdtype="batch_action" data-acttype="ajax" data-uri="{:U('youhuiquan/add_time')}" data-name="id" data-msg="{:L('confirm_addtime')}" value="一键延时" />
        <input type="button" class="btn" data-tdtype="batch_action" data-acttype="ajax" data-uri="{:U('youhuiquan/delete')}" data-name="id" data-msg="{:L('confirm_delete')}" value="{:L('delete')}" />
        <div id="pages">{$page}</div>
    </div>

</div>
<include file="public:footer" />
<script>
$('.J_preview').preview(); //查看大图
</script>
</body>
</html>