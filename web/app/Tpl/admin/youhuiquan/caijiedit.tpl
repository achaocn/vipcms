...
<div class="pad_lr_10" >
	<form id="info_form" action="{:U('items/caijiedit')}" name="searchform" method="post" >
	...
	<tbody>
		...
		<tr>
			<th>所属分类：</th>
			<td>
				<select class="J_cate_select mr10" data-pid="0" data-uri="{:U('items_cate/ajax_getchilds', array('type'=>0))}" data-selected="{$selected_ids}"></select>（不选为所有）
		...
		<tr>
			...
			<td>
                ...
                <option value="1000" <if condition="$info.page eq 1000"> selected</if> >采集全部</option>
				<?php for($a=1;$a<=100;$a++){?>
