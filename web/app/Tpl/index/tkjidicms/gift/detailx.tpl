...
<include file="public:header" />
    ...
    <div class="v_cont">
        ...
        <div class="reward">
            <div class="r_info">
                <img src="{:attach(str_replace('_s.'.array_pop(explode('.', $item['img'])), '_b.'.array_pop(explode('.', $item['img'])), $item['img']), 'score_item')}" alt="{$item.title}" width="400" height="400">
                <div class="r_info_in">
                    <div class="r_title">{$item.title}</div>
                    <div class="r_value">
                        价值：<span class="num">{$item.price}元</span>
                        <span class="val_right">所需积分：<span>{$item.score}</span>积分</span>
                    </div>
                    <div class="r_value2">
                        已兑换人数：<span class="num">{$item['order_count']}</span>名
                        <span class="r_join">剩余份数：<em class="org_2">{$item.stock}</em> 份</span>
                    </div>
                    <p>兑换限制：每名会员可兑换<em class="org_2">{$item.user_num}</em></span>次</p>
                   <!-- <a class="r_click J_gift_btn" data-id="{$item.id}" data-num="#J_num_{$item.id}" href="javascript:;">我要兑换</a>-->
                    <a href="javascript:;"   class="btn fl mr15 J_gift_btn" data-id="{$item.id}" data-num="#J_num_{$item.id}">我要兑换</a>
                    <div class="r_time"> 
                        结束时间：{$item.end_time|date="y年m月d日 H时s分",###}
                    </div>
            ...
            <div class="r_tab">
                ...
                <span class="good2">抽奖规则</span>
                <!--<span class="good3">抽奖记录</span>-->
            </div>
            <div class="tab_con">
...
<script src="__STATIC__/tkjidicms/js/city.min.js"></script>
<script>
    $(function() {
