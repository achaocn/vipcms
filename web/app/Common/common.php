...
function zhuan_counplink($pid=0,$link="",$goods_id=0,$commission_type=0)
{
	if(empty($pid))
	{
		return $link;
	}
	$link=str_replace(array('sellerId','activityId'),array('seller_id','activity_id'),$link);
	preg_match('/activity_id=([A-Za-z0-9]+)/', $link, $matche);
	$couponLink='http://uland.taobao.com/coupon/edetail?activityId='.$matche[1].'&itemId='.$goods_id.'&pid='.$pid.'&src=tkjd_tkjdzs&dx='.($item['commission_type']==1?1:"");
	return $couponLink;
}
function pdchicun($thumb,$dir='',$width=310)
{
	$img=$thumb;
	if (false !== strpos($img, 'taobaocdn.com') || false !== strpos($img, 'taobao.com') || false !== strpos($img, 'alicdn.com')|| false !== strpos($img, 'tbcdn.cn')) {
	
		$isMatched = preg_match('/(_)\d{3}(x)\d{3}/', $thumb);
	if($isMatched)
	{
		$thumb=preg_replace("/\d{3}(x)\d{3}/is", $width."x".$width, $thumb);
	}
	else
	{
		$thumb=$thumb.'_'.$width.'x'.$width.'.jpg';
...
*获取淘宝商品页
*/
function get_pro_con($taobaoid=0)
{
    if(empty($taobaoid))
    {
        return "";
    }
    $URL="http://hws.m.taobao.com/cache/mtop.wdetail.getItemDescx/4.1/?data={%22item_num_id%22%3A%22".$taobaoid."%22}";
        $curl = curl_init();
        ...
        if($json)
        {
            foreach($json['data']['images'] as $tt)
            {
                $con.='<img src="'.$tt.'" /><br />';
            }
            //$con=implode("<br />",$json['data']['images']);
        }
        return $con;
}
function changepic($img,$dir='',$w=300)
{
	//gd1.alicdn.com
	if (false !== strpos($img, 'taobaocdn.com') || false !== strpos($img, 'taobao.com') || false !== strpos($img, 'alicdn.com')|| false !== strpos($img, 'tbcdn.cn')) 
	{
		...
		$thumb=$img;					
		$isMatched = preg_match('/(_)\d{2,3}(x)\d{2,3}/', $thumb);
		if($isMatched)
		{
			$thumb=preg_replace("/\d{2,3}(x)\d{2,3}/is", $w."x".$w, $thumb);
		}
		else
		{
			$thumb=$thumb.'_'.$w.'x'.$w.'.jpg';
		}
	...
	else
	{
		...
		{
			return __ROOT__ . '/' . C('ftx_attach_path') . $dir.'/' . $img;
		}
	}
...
{
	$name_allow=mb_substr($name,0,2,'utf-8');
...
function qrcode($url='http://www.tkjidi.com',$level=3,$size=3.0){
   
      include('./app/Extend/Vendor/phpqrcode/phpqrcode.php');
...
function geturl($url)
{
	preg_match('/[\\w][\\w-]*\\.(?:com\\.cn|net.cn|com|cn|co|net|org|gov|cc|biz|info|wang|xyz|ren|top|xin|shop|club|win|site|vip|store|ren|online|biz|gift|name)(\\/|$)/isU', $url, $_var_2);
	$_var_3 = rtrim($_var_2[0], '/');
	return $_var_3;
}
function safe_replace($string) {
	
	 $array=array('select','insert','update','delete','and','order');
	 $string=str_replace(array('select','insert','update','delete','and','order'),array('','','','','',''),$string);
